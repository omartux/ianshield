...
void setup() {                    //esta funcion solo se ejecuta una sola vez
  ...
  while (condicion == 0) {        //si se cumple la condicion que es cero entrar a este bucle
    if (Serial.available() > 0) { //si hay algun dato en el puerto serial entrar a esta condicional if
      ...
      condicion = 1;              //despues de introducir el valor de "a" cambia la condicion por 1 para salir del bucle while inicial.
    }
  }
  ...
  Serial.println();
  Serial.println("Fin del programa, presiona reset en el Arduino");
  Serial.println();
  Serial.println();
}

void loop() {
  //en este programa no usamos la funcion loop() ya que solo queremos que se ejecute una sola vez
  //por eso solo usamos la funcion setup() ya que solo corre una sola vez.
}
